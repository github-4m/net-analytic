...
public class NetworkAccessServiceBean implements NetworkAccessService {
	...
	public List<List<Integer>> findTotalPerCluster() throws Exception {
		Integer totalCentroid = Integer.parseInt(
				getSystemParameterRepository().findByCode(NetworkAccessServiceBean.SP_TOTAL_CENTROID).getValue());
	...
	public List<List<Long>> cluster() throws Exception {
		...
		Integer totalIndividual = Integer.parseInt(
				getSystemParameterRepository().findByCode(NetworkAccessServiceBean.SP_TOTAL_INDIVIDUAL).getValue());
		Integer totalCentroid = Integer.parseInt(
				getSystemParameterRepository().findByCode(NetworkAccessServiceBean.SP_TOTAL_CENTROID).getValue());
		Integer totalAttribute = Integer.parseInt(
				getSystemParameterRepository().findByCode(NetworkAccessServiceBean.SP_TOTAL_ATTRIBUTE).getValue());
		Integer defaultIteration = Integer.parseInt(
				getSystemParameterRepository().findByCode(NetworkAccessServiceBean.SP_DEFAULT_ITERATION).getValue());
		...
		GeneticAlgorithm geneticAlgorithm = new GeneticAlgorithm(networkAccesses, maxValueAttributes, totalAttribute,
				totalCentroid, totalIndividual);
